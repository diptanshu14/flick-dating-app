...
export const signup = async (req: Request, res: Response) => {
    ...
    if (age < 18) {
        res.status(400).json({ success: false, message: "You must be atleast 18 years old" })
        return
    }
    ...
    try {
        const response = await signupService(name, email, password, age, gender, genderPreference)
        if (!response.user) {
